...
function App() {
  ...
  const getAllPosts = useHttp<PostType[]>(postAPI.getAllPosts, []);
  const posts = useAppSelector((state) => state.post);

  useEffect(() => {
    const posts = getStorage("post");
    if (!posts) {
      getAllPosts.sendRequest();
    } else {
  ...
  if (
    getAllPosts.data &&
    !getAllPosts.error &&
    getAllPosts.status === "completed"
  ) {
    dispatch(
      postActions.setPost({
        filteredPosts: getAllPosts.data,
        initialPosts: getAllPosts.data,
      })
    );
  }
  ...
  return (
    <main className="board">
      <Search />
      {getAllPosts.status === "completed" && getAllPosts.error && (
        <p>{getAllPosts.error}</p>
      )}
      ...
      {((getAllPosts.status === "completed" && !getAllPosts.error) ||
        posts) && <PostList />}
      {getAllPosts.status === "loading" && <LoadingSpinner />}
    </main>
  );
}
